      ...
      <div className="flex flex-col items-center lg:items-start justify-center w-full max-w-container gap-8">
        ...
        <div className="flex flex-col items-center justify-center xl:items-start min-w-full gap-2.5">
          ...
          <h4 className="text-xl max-lg:text-center font-regular text-slate-50">
            <AnimatedElement animation="slide-right" delay={0.1} className="w-full">
        ...
        {/* cards */}
        <div className="w-full grid grid-cols-1 grid-rows-4 md:grid-cols-2 md:grid-rows-2 xl:grid-cols-4 xl:grid-rows-1 place-items-center xl:place-items-start gap-8">
          {services.map(({ iconName, title, description }, index) => (
            <AnimatedElement key={index} animation="slide-up" delay={0.2 + (index * 0.5)} className="size-full grid place-items-center">
